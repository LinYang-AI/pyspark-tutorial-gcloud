...
def analyze(spark, format="parquet", gcs_input_path=None, gcs_output_path=None):
    ...
    df_enriched = df.withColumn("duration_time_in_minutes",(unix_timestamp(df["tpep_dropoff_datetime"])- unix_timestamp(df["tpep_pickup_datetime"]))60,)
    ...
    df_month.repartition(1).write.mode("overwrite").format("csv").option("header", "true").save(f"{​​​​​​gcs_output_path}​​​​​​/month_analysis")

    df_day = df_enriched.groupBy(dayofmonth("tpep_pickup_datetime").alias("dayofmonth")).agg(avg("duration_time_in_minutes").alias("average_trip_time_in_minutes")).orderBy("dayofmonth", ascending=True)

    df_day.repartition(1).write.mode("overwrite").format("csv").option("header", "true").save(f"{​​​​​​gcs_output_path}​​​​​​/day_analysis")
